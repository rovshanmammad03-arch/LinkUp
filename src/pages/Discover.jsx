    ...
    return (
        <div className="max-w-7xl mx-auto px-6 anim-up">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-6">
                <div>
                    <h1 className="text-4xl font-bold text-white mb-2 tracking-tight">{t('discover.title')}</h1>
                ...
                <div className="flex flex-col items-end gap-6">
                    <div className="relative flex p-1 bg-white/5 rounded-xl border border-white/5 shadow-inner">
                        <div 
                            className="absolute top-1 bottom-1 w-[calc(50%-4px)] bg-[#1a1a1a] rounded-lg shadow-xl border border-white/5 transition-transform duration-300 ease-in-out"
                            style={{ transform: tab === 'people' ? 'translateX(0)' : 'translateX(calc(100% + 8px))' }}
                        />
                        <button 
                            onClick={() => setTab('people')} 
                            className={`relative z-10 px-8 py-2.5 rounded-lg text-sm font-bold transition-colors duration-300 w-1/2 ${tab === 'people' ? 'text-white' : 'text-neutral-500 hover:text-neutral-300'}`}
                        >
                            {t('discover.people')}
                        </button>
                        <button 
                            onClick={() => setTab('projects')} 
                            className={`relative z-10 px-8 py-2.5 rounded-lg text-sm font-bold transition-colors duration-300 w-1/2 ${tab === 'projects' ? 'text-white' : 'text-neutral-500 hover:text-neutral-300'}`}
                        >
                            {t('discover.projects')}
                        </button>
                    </div>
            ...
            {tab === 'people' && (
                <div className="flex flex-col lg:flex-row gap-4 mb-12">
                    <div className="flex-1 relative group">
                        <Icon icon="mdi:magnify" className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-500 text-xl group-focus-within:text-brand-400 transition-colors" />
                        <input 
                            type="text" 
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            placeholder={t('discover.searchPlaceholder')} 
                            className="w-full bg-[#111] border border-white/5 rounded-2xl pl-12 pr-6 py-4 text-sm text-white placeholder-neutral-500 focus:outline-none focus:border-brand-500/50 focus:bg-[#151515] transition-all shadow-xl shadow-black/20"
                        />
                    </div>
            ...
            {tab === 'people' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredUsers.map((u, i) => {
                        ...
                        return (
                            <div key={u.id} className="bg-[#111]/80 backdrop-blur-xl border border-white/5 rounded-[32px] p-7 anim-up flex flex-col group hover:border-white/10 transition-all duration-300 shadow-2xl" style={{ animationDelay: `${i * 0.05}s` }}>
                                <div className="flex items-start justify-between mb-6">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${u.grad} flex items-center justify-center text-lg font-bold shrink-0 shadow-lg shadow-black/40 ring-4 ring-white/5`}>
                                ...
                                <div className="flex flex-wrap gap-2 mb-8 flex-1 content-start">
                                    {u.skills?.slice(0, 4).map((sk, si) => (
                                        <span key={si} className="px-3 py-1.5 bg-[#1a1a1a] border border-white/5 rounded-xl text-[10px] font-semibold text-neutral-400 group-hover:text-neutral-200 transition-colors">
                                            {sk.n}
                                        </span>
                                    ))}
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                    <button 
                                        ...
                                        className="flex-1 py-3 px-4 bg-white/5 border border-white/5 rounded-2xl text-[11px] font-bold text-white hover:bg-white/10 transition-all uppercase tracking-wider active:scale-95"
                                    >
                                        {t('discover.profile')}
                                    </button>
                                    <button 
                                        onClick={() => handleMessage(u.id)}
                                        className="w-12 h-12 flex items-center justify-center rounded-2xl border border-white/8 text-neutral-500 hover:text-white hover:bg-white/8 hover:border-white/15 transition-all active:scale-95"
                                    >
                                        <Icon icon="mdi:chat-processing-outline" className="text-xl" />
                                    </button>
                                    <button 
                                        onClick={() => handleFollow(u.id)}
                                        className={`w-12 h-12 flex items-center justify-center rounded-2xl border transition-all active:scale-95 ${isFollowing ? 'border-white/15 text-white bg-white/8' : 'border-white/8 text-neutral-500 hover:text-white hover:bg-white/8 hover:border-white/15'}`}
                                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    {filteredProjects.map((p, i) => {
                        ...
                        return (
                            <div key={p.id} className="bg-[#111]/60 backdrop-blur-xl border border-white/5 rounded-[32px] p-7 group hover:border-white/10 transition-all anim-up shadow-2xl flex flex-col" style={{ animationDelay: `${i * 0.05}s` }}>
                                <div className="flex items-start justify-between mb-6">
                                    <div className="flex items-center gap-5">
                                        <div className={`w-14 h-14 rounded-[18px] bg-gradient-to-br ${p.grad} flex items-center justify-center shrink-0 shadow-lg shadow-black/40`}>
                                            <Icon icon="mdi:folder-star-outline" className="text-white text-2xl" />
                                        </div>
                                        <div className="min-w-0">
                                            <h3 className="text-lg font-bold text-white mb-1 leading-tight group-hover:text-brand-300 transition-colors">{p.title}</h3>
                                            <p className="text-[11px] text-neutral-500 font-medium">
                                                {author?.name} · {p.team}
                                            </p>
                                        </div>
                                    </div>
                                    <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest ${p.status === 'completed' ? 'text-emerald-400 bg-emerald-400/10' : 'text-brand-400 bg-brand-400/10'}`}>
                                ...
                                <div className="flex flex-wrap gap-2 mb-8">
                                    {p.skills?.slice(0, 3).map((sk, si) => (
                                        <span key={si} className="px-3 py-1.5 bg-black/40 border border-white/5 rounded-xl text-[10px] font-semibold text-neutral-500 group-hover:text-neutral-300 transition-colors">{sk}</span>
                                    ))}
                                </div>

                                <div className="flex items-center justify-end mt-auto pt-4 border-t border-white/5 relative">
                                    {p.authorId === currentUser?.id ? (
                                        <div className="flex items-center gap-4">
                                            <div className="relative">
                                                <button 
                                                    onClick={() => setOpenOptionsId(openOptionsId === p.id ? null : p.id)}
                                                    className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${openOptionsId === p.id ? 'bg-white/10 text-white' : 'text-neutral-600 hover:text-white hover:bg-white/5'}`}
                                                >
                                                ...
                                                {openOptionsId === p.id && (
                                                    <>
                                                        ...
                                                        <div className="absolute bottom-full right-0 mb-2 w-48 bg-[#0a0a0a] border border-white/10 rounded-2xl shadow-2xl py-2 z-50 anim-up overflow-hidden">
                                                            <button 
                                                                onClick={() => handleToggleStatus(p.id)}
                                                                className="w-full px-4 py-2.5 flex items-center gap-3 text-xs font-bold text-neutral-400 hover:text-white hover:bg-white/5 transition-all text-left"
                                                            >
                                            ...
                                            <button 
                                                ...
                                                className="px-6 py-2 bg-white/5 border border-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-bold transition-all active:scale-95"
                                            >
                                        ...
                                        <button 
                                            ...
                                            className={`px-8 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg active:scale-95 ${p.applicants?.includes(currentUser?.id) ? 'bg-white/10 text-neutral-500 cursor-default' : 'bg-brand-500 hover:bg-brand-600 text-white shadow-brand-500/20'}`}
                                        >
